...
import { DrawingObject, FlashcardObject, TranslationObject, FreehandObject, StickyNoteObject, ImageObject, YouTubeVideoObject, EmojiObject, ToolType } from '../../lib/types/drawing';
...
export const DrawingObjectRenderer: React.FC<DrawingObjectRendererProps> = ({
  ...
  switch (object.type) {
    ...
    case 'emoji':
      const emojiObj = object as EmojiObject;
      
      const handleEmojiUpdate = (id: string, updates: Partial<EmojiObject>) => {
        if (onUpdate) {
          onUpdate(id, updates as Partial<DrawingObject>);
        }
      };
      ...
      return (
        <foreignObject
          x={object.x}
          y={object.y}
          width={emojiObj.width}
          height={emojiObj.height}
          style={{
            cursor: object.selected ? 'move' : 'pointer',
            overflow: 'visible',
            pointerEvents: currentTool === 'freehand' ? 'none' : 'all'
